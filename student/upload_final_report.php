...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['final_report'])) {
    $sid = $_SESSION['sid'];
    $uploadDir = "../final_reports/"; // Directory to store final reports
    $fileName = $_FILES['final_report']['name'];
    $tempName = $_FILES['final_report']['tmp_name'];
...
$startDate = date("Y-m-d");
   // You may want to add more checks and validations on the uploaded file, such as file type, size, etc.
    ...
    if (move_uploaded_file($tempName, $uploadDir . $fileName)) {
        ...
        $insertQuery = "INSERT INTO projects (sid, lid, project_title, start_date, final_report, project_status) VALUES ('$sid','$lid', '$projectTitle', '$startDate', '$uploadDir$fileName', 'unread')";
