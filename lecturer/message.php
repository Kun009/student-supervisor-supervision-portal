...
function getStudentName($con, $lid) {
    $query = "SELECT firstname, lastname FROM lecturer WHERE lid = '$lid'";
...
$unreadMessagesQuery = "SELECT m.*, CONCAT(l.firstname, ' ', l.lastname) as sender_name 
                        FROM messages m
                        JOIN student l ON m.sender_id = l.sid
                        WHERE m.recipient_id = '{$_SESSION['lid']}' AND m.is_read = 0";
...
$readMessagesQuery = "SELECT m.*, CONCAT(l.firstname, ' ', l.lastname) as sender_name 
                      FROM messages m
                      JOIN student l ON m.sender_id = l.sid
                      WHERE m.recipient_id = '{$_SESSION['lid']}' AND m.is_read = 1";
...
if (isset($_POST['recipient_id']) && isset($_POST['message'])) {
    ...
    $insertQuery = "INSERT INTO messages (sender_id, recipient_id, message) VALUES ('{$_SESSION['lid']}', '$recipientId', '$messageContent')";
...
$sentMessagesQuery = "SELECT * FROM messages WHERE sender_id = '{$_SESSION['lid']}'";
