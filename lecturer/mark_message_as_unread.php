...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['message_id'])) {
        ...
        $updateQuery = "UPDATE messages SET is_read = 0 WHERE message_id = '$messageId' AND recipient_id = '{$_SESSION['lid']}'";

        $updateResult = mysqli_query($con, $updateQuery);

        if ($updateResult) {
            // Fetch the updated message details to display in the "Unread Messages" section
            $fetchQuery = "SELECT sender_name, subject FROM messages WHERE message_id = '$messageId' AND recipient_id = '{$_SESSION['lid']}'";
