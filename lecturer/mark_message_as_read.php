...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['message_id'])) {
        ...
        $updateQuery = "UPDATE messages SET is_read = 1 WHERE message_id = '$messageId' AND recipient_id = '{$_SESSION['lid']}'";

        $updateResult = mysqli_query($con, $updateQuery);

        if ($updateResult) {
            // Fetch the updated message details to display in the "Read Messages" section
            $fetchQuery = "SELECT sender_name, subject, message FROM messages WHERE message_id = '$messageId' AND recipient_id = '{$_SESSION['lid']}'";
